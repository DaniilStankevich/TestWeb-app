...
const Sort: React.FC = React.memo(() => {
  ...
  const OnClickListItem = (obj: SortItem, objState: SortItem) => {
    ...
    if (obj.name === objState.name) {
      ...
      let newSent = list.filter(
        (el) => el.name === obj.name && el.sortProperty !== obj.sortProperty
      );
    ...
    <div className="sort" ref={sortRef}>
      <div className="sort__label">
        ...
        {sort.sortProperty.includes("-") ? (
          <IoArrowDownSharp className="sort__popup_icon" />
        ) : (
          <IoArrowUpSharp className="sort__popup_icon" />
        )}
      </div>
